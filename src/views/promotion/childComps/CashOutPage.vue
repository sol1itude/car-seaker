<template>
  <div class="cash-out-page" @touchmove.prevent>
    ...
    <div class="cash-out-input-container">
      <div>
        <img style="margin-left: -60px;margin-top: 20px;vertical-align: 13px;width: 25px"
      ...
      <div v-if="cashOutType==='ali'" class="remember-ali-account">
        <div style="height: 100%;width: 100%;display: inline-block">
          ...
          <img @click="rememberAliAccount=!rememberAliAccount" style="margin-left: 5px;height: 18px;width: 18px"
              ...
              alt="hello">
          <span @click="rememberAliAccount=!rememberAliAccount" style="position: relative;top: 2px;left: 8px">记住账号</span>
        </div>
      </div>
    </div>
    <div @click="cashOutNow" id="cash-out-now-btn" style="width: 240px">立即提现</div>
  </div>
</template>
  ...
  methods:{
    ...
    cashOutNow(){
      ...
      if (this.judgeAliAccount()){
        alert('真的要用 '+this.cashOutType+' 提现吗？')
      }else {
        alert('支付宝账号为空')
      }
    }
